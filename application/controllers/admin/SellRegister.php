...
class SellRegister extends CI_Controller {
    ...
    public function index() {
		...
		$data = array();
        $data['page'] = 'Sell Register';
        if (isset($this->session->userdata['is_admin_login'])) {
            $data['seller_register_list'] = $this->select_model->get_seller_register();
            $this->load->view('admin/controls/vwHeader');
            $this->load->view('admin/controls/vwLeft',$data);
            $this->load->view('admin/vwManageSellRegister',$data);
