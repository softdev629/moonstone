            ...
            if ($err) {
              ...
              if($data_array){
                 foreach ($data_array as $data_key => $data_value) {
                        ...
                        $this->db->insert($tblName, $cron_data);
                        $cron_tbl_id = $this->db->insert_id();
                        $data=array(
                            'status'=>2,
                            'date_updated'=>date("Y-m-d")
                        );
					   $this->update_model->update_new_plate_status($data, $prefix, $numbers, $letters,$cron_tbl_id);
					   $this->update_model->update_new_prefix_status($data, $prefix, $numbers, $letters,$cron_tbl_id);
